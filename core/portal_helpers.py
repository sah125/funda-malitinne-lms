...
def get_urgent_actions():
    ...
    if learners_without_poe > 0:
        actions.append({
            'priority': 'critical',
            'icon': '📄',
            'description': 'Learners missing Portfolio of Evidence',
    ...
    if inactive_learners > 0:
        actions.append({
            'priority': 'medium',
            'icon': '👤',
            'description': 'Inactive learners (7+ days)',
    ...
    if overdue_submissions > 0:
        actions.append({
            'priority': 'high',
            'icon': '📝',
            'description': 'Overdue assessments waiting for grading',
    ...
    if missing_consent > 0:
        actions.append({
            'priority': 'medium',
            'icon': '🔒',
            'description': 'Learners missing POPIA consent',
